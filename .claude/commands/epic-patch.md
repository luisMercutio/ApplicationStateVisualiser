Apply a small targeted patch to an already-implemented Epic. Usage: `/epic-patch EPIC-002`
...
You are guiding the user through a scoped change to an existing, implemented Epic — a business-rule tweak, a new validation, a UI adjustment, or a small field addition. You do NOT re-run the architect agents. If the change is purely one new Business Rule that fits inside this Epic (no new entity/table/endpoint-group/slice/route), prefer `/br-add EPIC-00X` instead — it appends a single BR without a full patch cycle. If the user's change touches the API contract or DB schema, you will escalate to a new Epic instead.

---

## Step 1 — Validate preconditions

1. Read `.claude/architecture/epics.md`. Confirm the target Epic (`$ARGUMENTS`) has `Status = Done`. If not, abort:
   > `$ARGUMENTS` is not yet implemented (status: `<STATUS>`). Patches can only be applied to `Done` Epics.

2. Read `<epic-folder>/suggestion.md` and `<epic-folder>/ClassDiagram.md` and `<epic-folder>/openapi.yaml` — you will need them to scope the session.
...
The full patch ID is `$ARGUMENTS-P<N>` (e.g. `EPIC-002-P1`). Use this throughout.

---

## Step 3 — Guided patch session

Summarise the parent Epic in 3–5 bullet points (title, BRs, endpoints, components) so the user has context.
...
2. **Affected BRs** — Which existing BR IDs does this touch? Are any new BRs added? State new ones explicitly. (If the whole change is a single new BR with no scaffolding, consider `/br-add $ARGUMENTS` instead of a patch.)
...
- If **yes** or **unsure** → stop and tell the user:
  > This patch touches the API contract or DB schema. Changes of that scope need a new Epic so the architect baseline chain stays consistent. Run `/epic-suggest EPIC-00X` for the next Epic instead.
...
- Once approved, re-run `/epic-patch $ARGUMENTS` — the command will detect the approved patch and proceed to implementation
...
Read the parent Epic artifacts needed for context:
...
Parent Epic baseline — ClassDiagram: <epic-folder>/ClassDiagram.md
Parent Epic baseline — openapi:      <epic-folder>/openapi.yaml
...
Patch description:  .claude/architecture/patches/<patch-ID>/patch-suggestion.md
Parent Epic baseline — FrontendState: <epic-folder>/FrontendState.md
Parent Epic baseline — openapi:       <epic-folder>/openapi.yaml
...
Patch description: .claude/architecture/patches/<patch-ID>/patch-suggestion.md
Parent testState:   <epic-folder>/testState.md
...
| Change type | Use |
|---|---|
| A single new Business Rule that fits an existing Epic (no new scaffolding) | `/br-add` |
| Business rule tweak, new validation, UI copy, minor field | `/epic-patch` |
| New endpoint, new DB column/table, new API response field | New Epic (`/epic-suggest`) |
| Cross-cutting refactor affecting multiple Epics | New Epic or human-led PR |
