...
async function ensureAppSchema(pool) {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS technical_specifications (
      ...
      CONSTRAINT fk_techspec_br FOREIGN KEY (business_rule_id) REFERENCES business_rules(id) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS technical_specification_entries (
      ...
      created_at                DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at                DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      KEY idx_tsentry_spec (technical_specification_id),
      CONSTRAINT fk_tsentry_spec FOREIGN KEY (technical_specification_id) REFERENCES technical_specifications(id) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS technical_specification_artifacts (
      ...
      created_at                DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at                DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      KEY idx_tsartifact_spec (technical_specification_id),
      CONSTRAINT fk_tsartifact_spec FOREIGN KEY (technical_specification_id) REFERENCES technical_specifications(id) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`);
}
...
async function listBusinessRules(id) {
  ensureReady();
  const pool = await appPool(id);
  const [rows] = await pool.query('SELECT * FROM business_rules ORDER BY seq IS NULL, seq, name');
  return rows.map(brRowToDto);
}

async function getBrRow(pool, brId) {
  const [rows] = await pool.query('SELECT * FROM business_rules WHERE id = ?', [brId]);
  return rows[0] || null;
}
  ...
  try {
    await pool.query(
      `INSERT INTO business_rules
         (id, name, epic_id, seq, rule, rationale, category, features, modifies_features, depends_on, touches, delta)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [brId, b.name, b.epicId, b.seq, b.rule, b.rationale, b.category,
...
async function deleteBusinessRule(id, brId) {
  ensureReady();
  const pool = await appPool(id);
  const [res] = await pool.query('DELETE FROM business_rules WHERE id = ?', [brId]);
  return res.affectedRows > 0;
}
...
// ── Technical specifications (per-BR implementation spec) ─────────────────────
// A technical specification is the parent entity for how a single Business Rule
// is implemented. It owns two child collections: entries (implementation
// instructions, each authored by a 'user' or an 'agent') and artifacts (the
...
const TS_SELECT = `
  SELECT ts.*, br.name AS br_name, br.seq AS br_seq
    FROM technical_specifications ts
    JOIN business_rules br ON br.id = ts.business_rule_id`;

const TS_SOURCES = new Set(['user', 'agent']);
const TS_CHANGE_TYPES = new Set(['add', 'modify', 'remove']);

function specEntryRowToDto(r) {
  return {
    id: r.id,
    technicalSpecificationId: r.technical_specification_id,
    description: r.description,
    source: r.source,
    createdAt: r.created_at,
    updatedAt: r.updated_at,
...
function specArtifactRowToDto(r) {
  return {
    id: r.id,
    technicalSpecificationId: r.technical_specification_id,
    kind: r.kind,
...
function specRowToDto(r, entries = [], artifacts = []) {
  return {
    id: r.id,
    businessRuleId: r.business_rule_id,
    title: r.title ?? null,
    ...
    brName: r.br_name ?? null,
    brSeq: r.br_seq ?? null,
    entries: entries.map(specEntryRowToDto),
    artifacts: artifacts.map(specArtifactRowToDto),
    createdAt: r.created_at,
    updatedAt: r.updated_at,
  };
}
...
async function listTechnicalSpecs(id) {
  ensureReady();
  const pool = await appPool(id);
  const [specs] = await pool.query(`${TS_SELECT} ORDER BY br.seq IS NULL, br.seq, ts.created_at`);
  ...
  const eBySpec = groupBy(entries);
  const aBySpec = groupBy(artifacts);
...
// ── Spec entries (implementation instructions, source = user | agent) ─────────
function normaliseSpecEntry(input) {
  const description = String(input?.description ?? '').trim();
  if (!description) throw badRequest('description is required');
  const source = String(input?.source ?? 'user').trim();
  if (!TS_SOURCES.has(source)) throw badRequest("source must be 'user' or 'agent'");
...
// ── Spec artifacts (the generated file changes the spec is parent to) ─────────
function normaliseSpecArtifact(input) {
  const kind = String(input?.kind ?? '').trim();
  const filePath = String(input?.path ?? '').trim();
  if (!kind) throw badRequest('kind is required');
  if (!filePath) throw badRequest('path is required');
  const changeType = String(input?.changeType ?? 'add').trim();
  if (!TS_CHANGE_TYPES.has(changeType)) throw badRequest("changeType must be 'add', 'modify' or 'remove'");
  return {
    kind, path: filePath, changeType,
...
module.exports = {
  ...
  listTechnicalSpecs, createTechnicalSpec, updateTechnicalSpec, deleteTechnicalSpec,
  createSpecEntry, updateSpecEntry, deleteSpecEntry,
  createSpecArtifact, updateSpecArtifact, deleteSpecArtifact,
