...
// mark which one is active. The active application is what every data view
// (Business Rules, Notes, snapshots) reads from.
@Component({
  selector: 'app-applications-manager-dialog',
  standalone: true,
  imports: [
    MatDialogModule, MatButtonModule, MatIconModule, MatTooltipModule,
    MatListModule, MatRadioModule, FormsModule,
  ],
  template: `
    ...
    <mat-dialog-content>
      ...
      @if (applications().length === 0) {
        ...
        <mat-radio-group [ngModel]="activeId()" (ngModelChange)="setActive($event)">
          <mat-list>
            @for (a of applications(); track a.id) {
              <mat-list-item class="row" [class.active]="a.id === activeId()">
                <mat-radio-button matListItemIcon [value]="a.id"
                                  matTooltip="Set active"></mat-radio-button>
